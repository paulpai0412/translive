...
RX uses `thread/realtime/appendSpeech` when native Traditional-Chinese audio is absent. Assistant transcript deltas are spoken in-order whenever the buffer reaches **8 Chinese characters** or a punctuation boundary. A temporary comma is added when an 8-character chunk has no punctuation so the speech backend treats it as a speakable phrase. The 8-character setting favors lower latency over perfectly smooth phrasing; a 100-chunk circuit breaker prevents recursive speech loops.
